...
export default function AdminPanel() {
  ...
  useEffect(() => {
    const verifyAdmin = async () => {
      try {
        ...
        if (!response.ok || data.email !== 'lord_izana@yahoo.com') {
  ...
  const handleBan = async (permanent = false) => {
    if (!selectedHunter || !banReason) return;
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-red-500 font-solo">Shadow Monarch Control Panel</h1>
          <button 
          ...
          <div className="lg:col-span-1">
            {selectedHunter ? (
              <div className="bg-gray-800 p-6 rounded-lg sticky top-4">
                ...
                <div className="mt-6">
                  <h3 className="font-bold mb-2">Shadow Army Control</h3>
                  <button 
                    className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded mb-2"
                    onClick={() => {
                      // Add to shadow army logic
                    }}
                  >
                    Add to Shadow Army
                  </button>
                  <button 
                    className="w-full bg-purple-800 hover:bg-purple-900 text-white font-bold py-2 px-4 rounded"
                    onClick={() => {
                      // Remove from shadow army logic
                    }}
                  >
                    Remove from Shadow Army
                  </button>
                </div>
              </div>
            ) : (
